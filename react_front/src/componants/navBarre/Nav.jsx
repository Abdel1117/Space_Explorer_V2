...
const Nav = () => {
  ...
  const [toogle, setNavbar] = useState(false);
  const navigate = useNavigate()

  const resetNavbar = () => {
    const innerWidth = window.innerWidth

    if (innerWidth >= 640) setNavbar(false)
  }

  useEffect(() => {
    window.addEventListener('resize', resetNavbar);

    return () => window.removeEventListener('resize', resetNavbar)
  }, [])
      ...
      <div className="bg-white dark:bg-[#1C1E21] shadow">
        <div className="container mx-auto px-4">
          <div className="flex items-center justify-between py-4">
            ...
            <div onClick={() => { setNavbar(!toogle) }} className="sm:hidden cursor-pointer ">
